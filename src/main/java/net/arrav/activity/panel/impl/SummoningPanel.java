...
public class SummoningPanel extends Panel {
	...
	public void update() {
		...
		fancyFont.drawLeftAlignedEffectString("Summoning Pouch Creation - Level: " + summoningLevel, beginX + 20, beginY + 31, 0xFF8A1F, 0);
		...
		for(int i = 0; i < familiars.length; i++) {
			...
			//smallFont.drawLeftAlignedEffectString((summoningLevel >= reqLvl ? "@or1@" : "@red@") + reqLvl, beginX + 12 + x, beginY + offset + 14, 0xFF8A1F, 0);
			smallFont.drawCenteredEffectString((summoningLevel >= reqLvl ? "@or1@Lvl " : "@red@Lvl ") + reqLvl, beginX + 44 + x, beginY + offset + 46, 0xFF8A1F, 0);
			offset += i % 6 == 5 ? 55 : 0;
		}
		if(!tooltip.isEmpty() && familiar != -1) {
			...
			smallFont.drawLeftAlignedEffectString(tooltip, client.mouseX + (off ? -(width + 14) : 12), client.mouseY + 9 + offsetY, 0xFF8A1F, 0);
