...
public final class CacheIndexMapped implements CacheIndex {
	...
	public CacheIndexMapped(MappedByteBuffer dat, MappedByteBuffer idx, int id) {
		index = id;
		dataFile = dat;
		indexFile = idx;
	}
	 ...
	 */
	public synchronized boolean writeFile(boolean flag, int id, int length, byte[] data) {
		try {
			int l;
			if(flag) {
				seekTo(indexFile, id * 6);
