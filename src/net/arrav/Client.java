...
import net.arrav.game.MapDecoder;
import net.arrav.game.Scene;
import net.arrav.game.WorldObjectSpawn;
import net.arrav.game.emitter.Particle;
import net.arrav.game.tile.EntityUnit;
import net.arrav.game.tile.GroundDecoration;
...
import net.arrav.od.OnDemandEntry;
import net.arrav.od.OnDemandFetcher;
import net.arrav.sign.SignLink;
import net.arrav.util.collect.LinkedDeque;
import net.arrav.util.collect.SinglyLinkableEntry;
import net.arrav.util.io.Buffer;
import net.arrav.util.io.ISAACCipher;
import net.arrav.util.string.StringEncoder;
import net.arrav.activity.Activity;
import net.arrav.activity.panel.impl.*;
import net.arrav.activity.ui.UI;
import net.arrav.cache.unit.*;
import net.arrav.game.model.*;
import net.arrav.game.tile.Wall;
import net.arrav.game.tile.WallDecoration;
import net.arrav.media.GraphicalComponent;
import net.arrav.media.img.BitmapImage;
import net.arrav.util.MouseTracker;
import net.arrav.util.WebToolkit;
...
public class Client extends ClientEngine {
	...
	public void determineMenuSize() {
		int w = (Config.def.menu() > 3 ? plainFont : boldFont).getEffectStringWidth("Choose Option");
		for(int j = 0; j < menuPos; j++) {
			...
			final int k = (Config.def.menu() > 3 ? plainFont : boldFont).getEffectStringWidth(menuItemName[j]);
			if(k > w) {
				w = k;
			}
		}
	...
	public void method47(boolean flag) {
		...
		for(int l = 0; l < count; l++) {
			...
			player.noTransform = (!Config.def.lowMem() && playerCount > 50 || playerCount > 200) && !flag && player.idleAnim == player.anInt1511;
				...
				if((j == 13 || j == 10) && chatInput.length() > 0) {
					...
					if(localPrivilege == 11) {
						...
						if(chatInput.equals("::msg1")) {
							...
							pushMessage("Chat message.", 1, 1, "Arrav");
							pushMessage("Received private message.", 2, 2, "Arrav");
							pushMessage("wishes to trade with you.", 4, "Arrav");
							pushMessage("Arrav has logged in/out.", 5, "");
							pushMessage("Sent private message.", 6, 6, "Arrav");
							pushMessage("Clan message.", 7, 7, "Clan Name:Clanmate Name");
							pushMessage("sent some request.", 8, "Arrav");
							pushMessage("yell message.", 9, "Arrav");
						}
						if(chatInput.equals("::msg2")) {
							pushMessage("Regular", 1, "Arrav");
							pushMessage("Respected-Member", 1, 1, "Arrav");
							pushMessage("Designer", 1, 2, "Arrav");
							pushMessage("Youtuber", 1, 3, "Arrav");
							pushMessage("Donator", 1, 4, "Arrav");
							pushMessage("Super-Donator", 1, 5, "Arrav");
							pushMessage("Extreme-Donator", 1, 6, "Arrav");
							pushMessage("Golden-Donator", 1, 7, "Arrav");
							pushMessage("Helper", 1, 8, "Arrav");
							pushMessage("Moderator", 1, 9, "Arrav");
							pushMessage("Senior-Moderator", 1, 10, "Arrav");
							pushMessage("Administrator", 1, 11, "Arrav");
						}
	...
	private void method142(int i, int j, int k, int l, int i1, int j1, int k1) {
		if(i1 >= 1 && i >= 1 && i1 <= 102 && i <= 102) {
			if(Config.def.lowMem() && j != cameraPlane) {
				return;
			}
			long hash = 0;
			if(j1 == 0) {
