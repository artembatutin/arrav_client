...
import net.arrav.cache.unit.OverlayFloorType;
...
import net.arrav.game.model.Entity;
...
public final class MapDecoder {
			...
			for(; ; ) {
				if(bool_255_) {
					...
					if(i_261_ > 0 && i_262_ > 0 && i_261_ < 103 && i_262_ < 103) {
						...
						if(i_260_ != 22 || Config.def.groundDec() || class46.hasActions || class46.aBoolean736) {
					...
					for(int y = -5; y < sizeY + 5; y++) {
						...
						if(y >= 1 && y < sizeY - 1) {
							...
							if(underlayFloId > 0 || overlayFloId > 0) {
								...
								int shadow = Config.def.enchanceMap() ? 52 : 96;
								if(hsl != -1) {
	...
	private void method175(int y, Scene worldController, CollisionMap class11, int j, int p, int x, int id, int j1, boolean oldMap) {
		...
		if(j == 22) {
			if(!Config.def.groundDec() && !loc.hasActions && !loc.aBoolean736) {
				return;
			}
			Object obj;
			if(loc.animationId == -1 && loc.childIds == null) {
