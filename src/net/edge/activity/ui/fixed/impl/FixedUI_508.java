...
public class FixedUI_508 extends FixedUI {
	...
	public void updateChat() {
		...
		if(client.messagePromptRaised) {
			...
			client.boldFont.drawCenteredString(client.amountOrNameInput + "*", 259, 80, 128);
	...
	public void updateMap() {
		...
		if(client.minimapOverlay == 2) {
			...
			ImageCache.get(238).drawImage(8, 123);
	...
	public void buildInventory() {
		...
		for(int i = 0; i < 13; i++) {
			if(client.clickX >= tabClickPositions474[i][0] && client.clickX <= tabClickPositions474[i][1] && client.clickY >= tabClickPositions474[i][2] && client.clickY < tabClickPositions474[i][3] && client.olderTabInterfaces[i] != -1) {
				client.invTab = i;
	...
	public void updateInventory() {
		...
		ImageCache.get(240).drawImage(0, 0);
	...
	private void displayOrb(int x, int y, int orb, boolean hover) {
		...
		ImageCache.get(orb == Constants.ORB_RUN && OrbHandler.runEnabled ? 73 : orb == Constants.ORB_HEALTH && OrbHandler.poisoned ? 78 : 56 + orb).drawImage(x + 3, y + 3);
